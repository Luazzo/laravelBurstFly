...
@section('page_title', __('voyager::generic.'.(isset($post->id) ? 'edit' : 'add')).' '.$dataType->display_name_singular)
...
@section('content')
    <div class="page-content container-fluid">
        <form class="form-edit-add" role="form" action="@if(isset($post->id)){{ route('voyager.posts.update', $post->id) }}@else{{ route('voyager.posts.store') }}@endif" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-8">
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            @include('voyager::multilingual.input-hidden', [
                                '_field_name'  => 'title',
                                '_field_trans' => get_field_translations($post, 'title')
                            ])
                            <input type="text" class="form-control" id="title" name="title" placeholder="{{ __('voyager::generic.title') }}" value="{{ $post->title ?? '' }}">
                        </div>
                    </div>
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            @include('voyager::multilingual.input-hidden', [
                                '_field_name'  => 'body',
                                '_field_trans' => get_field_translations($post, 'body')
                            ])
                            @php
                                $dataTypeRows = $dataType->{(isset($post->id) ? 'editRows' : 'addRows' )};
                                $row = $dataTypeRows->where('field', 'body')->first();
                            @endphp
                            {!! app('voyager')->formField($row, $dataType, $post) !!}
                        </div>
                    </div><!-- .panel -->
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            @include('voyager::multilingual.input-hidden', [
                                '_field_name'  => 'excerpt',
                                '_field_trans' => get_field_translations($post, 'excerpt')
                            ])
                                ...
                                @if(!in_array($row->field, $exclude))
                                    ...
                                    @else
                                        <div class="form-group @if($row->type == 'hidden') hidden @endif @if(isset($display_options->width)){{ 'col-md-' . $display_options->width }}@endif" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                            ...
                                            @foreach (app('voyager')->afterFormFields($row, $dataType, $post) as $after)
                                                {!! $after->handle($row, $dataType, $post) !!}
                                            @endforeach
                                        </div>
                                    @endif
                                @endif
                    ...
                    <div class="panel panel panel-bordered panel-warning">
                        ...
                        <div class="panel-body">
                            <div class="form-group">
                                ...
                                @include('voyager::multilingual.input-hidden', [
                                    '_field_name'  => 'slug',
                                    '_field_trans' => get_field_translations($post, 'slug')
                                ])
                                <input type="text" class="form-control" id="slug" name="slug"
                                       placeholder="slug"
                                       {!! isFieldSlugAutoGenerator($dataType, $post, "slug") !!}
                                       value="{{ $post->slug ?? '' }}">
                            </div>
                            <div class="form-group">
                                <label for="status">{{ __('voyager::post.status') }}</label>
                                <select class="form-control" name="status">
                                    <option value="PUBLISHED"@if(isset($post->status) && $post->status == 'PUBLISHED') selected="selected"@endif>{{ __('voyager::post.status_published') }}</option>
                                    <option value="DRAFT"@if(isset($post->status) && $post->status == 'DRAFT') selected="selected"@endif>{{ __('voyager::post.status_draft') }}</option>
                                    <option value="PENDING"@if(isset($post->status) && $post->status == 'PENDING') selected="selected"@endif>{{ __('voyager::post.status_pending') }}</option>
                                </select>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="checkbox" name="featured"@if(isset($post->featured) && $post->featured) checked="checked"@endif>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="panel-body">
                            <div class="form-group">
                                ...
                                @include('voyager::multilingual.input-hidden', [
                                    ...
                                    '_field_trans' => get_field_translations($post, 'meta_description')
                                ])
                            ...
                            <div class="form-group">
                                ...
                                @include('voyager::multilingual.input-hidden', [
                                    ...
                                    '_field_trans' => get_field_translations($post, 'meta_keywords')
                                ])
                            ...
                            <div class="form-group">
                                ...
                                @include('voyager::multilingual.input-hidden', [
                                    ...
                                    '_field_trans' => get_field_translations($post, 'seo_title')
                                ])
            ...
            <button type="submit" class="btn btn-primary pull-right">
                @if(isset($post->id)){{ __('voyager::post.update') }}@else <i class="icon wb-plus-circle"></i> {{ __('voyager::post.new') }} @endif
            </button>
        </form>
